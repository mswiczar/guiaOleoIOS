...
-(void) showData
{
	...
	if(self.is_yellow)
	{
		[butoncall setBackgroundImage:[UIImage imageNamed:@"yp.png"] forState:0];
		[butoncall setNeedsDisplay];
	}
	
	itemnumber.text = [NSString stringWithFormat:@"%d", thestore.arow] ; 
	itemName.text =[NSString stringWithFormat:@"%@", thestore.title] ; 
	
//	itemAddress1.text = [NSString stringWithFormat:@"%@ %@", thestore.adress_street ,thestore.adress_cnt ] ;  
	itemAddress1.text = [NSString stringWithFormat:@"%@", thestore.adress_street] ;  
	itemAddress2.text = [NSString stringWithFormat:@"%@ %@", thestore.adress_pcd , thestore.adress_city ] ;  
	if (thestore.km ==nil)
	{
		itemKM.text =@"";
	...
	{
		if ( [thestore.km isEqualToString:@""]==NO) 
		{
			itemKM.text = [NSString stringWithFormat:@"%@ KM", thestore.km ] ;  
		}
		else
		{
			itemKM.text =@"";
